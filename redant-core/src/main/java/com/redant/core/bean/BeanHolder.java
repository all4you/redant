...
public class BeanHolder {
    ...
    public void initBeans(){
        ...
        synchronized (BeanHolder.class){
            if(null==beanHolderMap) {
                LOGGER.info("[BeanHolder] Start init bean classes...currentThread={}", ThreadUtil.currentThreadName());
                try {
                    /*
                    ...
                    beanHolderMap = new LinkedHashMap<>(classSet.size()+1);
                        ...
                        for (Class<?> cls : classSet) {
                            ...
                            if (bean != null) {
                                ...
                                if(beanHolderMap.containsKey(beanName)){
                                    LOGGER.warn("[BeanHolder] Duplicate bean with name={}",beanName);
                                    continue;
                                }
                                beanHolderMap.put(beanName, cls.newInstance());
                            }
                        }
                        LOGGER.info("[BeanHolder] Init bean classes success!");
                    }else{
                        LOGGER.warn("[BeanHolder] No bean classes scanned!");
                    }
                } catch (RuntimeException e) {
                    throw e;
                } catch (Exception e) {
                    LOGGER.error("[BeanHolder] Init bean classes error,cause:",e);
                }
            }
            beanLoaded = true;
        }
    ...
    public void annotationInject(){
        if(beanLoaded()){
            LOGGER.info("[BeanHolder] Start annotationInject...");
            for(Map.Entry<String,Object>  entry : beanHolderMap.entrySet()){
            ...
            LOGGER.info("[BeanHolder] annotationInject success!");
        }
    }
