        ...
        try {
            if(client.checkExists().forPath(ZkNode.ROOT_NODE_PATH)!=null){
                ...
                for(String child : children){
                    ...
                    if(node !=null){
                        LOGGER.info("add slave={} to slaveNodeMap when init", node);
        ...
        public void childEvent(CuratorFramework client, PathChildrenCacheEvent event) throws Exception {
            ...
            if(node ==null){
                ...
                switch (event.getType()) {
                    case CHILD_ADDED:
                        slaveNodeMap.put(node.getId(), node);
                        LOGGER.info("CHILD_ADDED with path={},data={},current slave size={}", data.getPath(), new String(data.getData(),CharsetUtil.UTF_8),slaveNodeMap.size());
                        break;
                    case CHILD_REMOVED:
                        slaveNodeMap.remove(node.getId());
                        LOGGER.info("CHILD_REMOVED with path={},data={},current slave size={}", data.getPath(), new String(data.getData(),CharsetUtil.UTF_8),slaveNodeMap.size());
                        break;
                    case CHILD_UPDATED:
                        slaveNodeMap.replace(node.getId(), node);
                        LOGGER.info("CHILD_UPDATED with path={},data={},current slave size={}", data.getPath(), new String(data.getData(),CharsetUtil.UTF_8),slaveNodeMap.size());
                        break;
                    default:
                        break;
                }
